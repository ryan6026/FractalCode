...
public:
    ...
    void plotMBPointBWLog(int x, int y, double n, double maxN)
    {
        double lohg = log(n)/log(maxN);
        SDL_SetRenderDrawColor(renderer, (int)(255*lohg), (int)(255*lohg), (int)(255*lohg), 255);
        SDL_RenderDrawPoint(renderer, x, y);
    }

    void plotMBPointBW(int x, int y, double n, double maxN)
    {
        SDL_SetRenderDrawColor(renderer, (int)(255*n/maxN), (int)(255*n/maxN), (int)(255*n/maxN), 255);
        SDL_RenderDrawPoint(renderer, x, y);
    }

    void plotMBPoint(int x, int y, double n, double maxN)
    {
        n = (int)maxN*pow((double)((double)n/maxN), gradCoef)*exp(((double)n-maxN)/maxN);
    ...
    void plotMBSetBW(double** n)
    {
        ...
        for(int x=0; x<SCREEN_WIDTH; x++)
        {
            ...
            {
                if(MAX!=0)
                    plotMBPointBW(x, y, n[x][y], MAX);
            }
        }
        SDL_RenderPresent(renderer);
    }

    void plotMBSetBWLog(double** n)
    {
        double MAX = getMax(n);
        for(int x=0; x<SCREEN_WIDTH; x++)
        {
            ...
            {
                if(MAX!=0)
                    plotMBPointBWLog(x, y, n[x][y], MAX);
            }
        }
        SDL_RenderPresent(renderer);
    }

    void plotMBSetBW(int** n)
    {
        int MAX = getMax(n);
        for(int x=0; x<SCREEN_WIDTH; x++)
        {
    ...
    {
        SDL_SaveBMP(screenSurface, "test.bmp");
        delay(500);
    }
