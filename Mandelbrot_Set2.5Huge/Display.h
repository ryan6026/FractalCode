...
class GUI {
  ...
  public:
    ...
    void plotMBPoint(int x, int y, int n, int maxN) {
        ...
        if(n<maxN/7) {
            SDL_SetRenderDrawColor(renderer, 255*sin(PI*n*7/(maxN*2))*sin(PI*n*7/(maxN*2)), 255-255*sin(PI*n*7/(maxN*2))*sin(PI*n*7/(maxN*2)), 255-255*sin(PI*n*7/(maxN*2))*cos(PI*n*7/(maxN*2)), 255);
    ...
    void Plot3MBPoint(int x, int y, int* nrgb){//nrgb[i+1] = maxN ||| nrgb[i] = N for i<6 (only three color channels
        int n, maxN;
        for(int i=0;i<6;i+=2){
            n = (int)nrgb[i+1]*pow((double)((double)nrgb[i]/nrgb[i+1]), gradCoef);//nrgb = {r, rmax, b, bmax, g, gmax}
            maxN = nrgb[i+1];
            if(n<maxN/7) {
                nrgb[i] = 255*sin(PI*n*7/(maxN*2))*sin(PI*n*7/(maxN*2));
            } else if(n>2*maxN/7&&n<3*maxN/7) {
                nrgb[i] = 255*sin(PI*n*7/(maxN*4))*sin(PI*n*7/(maxN*4));
            } else if(n>3*maxN/7&&n<4*maxN/7) {
                nrgb[i] = 255*sin(PI*n*7/(maxN*6))*sin(PI*n*7/(maxN*6));
            } else if(n>4*maxN/7&&n<5*maxN/7) {
                nrgb[i] = 255*sin(PI*n*7/(maxN*8))*sin(PI*n*7/(maxN*8));
            } else if(n>5*maxN/7&&n<6*maxN/7) {
                nrgb[i] = 255*sin(PI*n*7/(maxN*10))*sin(PI*n*7/(maxN*10));
            } else {
                nrgb[i] = 255*sin(PI*n*7/(maxN*12))*sin(PI*n*7/(maxN*12));
            }
        }
        SDL_SetRenderDrawColor(renderer, nrgb[0], nrgb[2], nrgb[4], 255);
        SDL_RenderDrawPoint(renderer, x, y);
    }

    void plot3MBset(){
        int* MAX; MAX = new int[3];// MAX = {maxr, maxg, maxb}
    }
